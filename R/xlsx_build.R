xlsx_build <- function(data, file) {
  ...
  psychographics <- data$full_dat |>
    ...
    dplyr::mutate(
      category = stringr::str_replace_all(category, "\\<b\\>|\\</b\\>", ""),
      question = stringr::str_replace_all(question, "\\<br\\>", " "),
      dplyr::across(dplyr::matches("sample|weighted"), f1),
      dplyr::across(dplyr::matches("vertical|horizontal"), f2)
    ) |>
    dplyr::rename_with(~ paste(., "%"), dplyr::matches("vertical|horizontal"))
  colnames(psychographics) <- stringr::str_to_title(gsub(
    ".*\\_",
    "",
    colnames(psychographics)
  ))

  segments <- data$dat |>
    dplyr::mutate(
      segment = stringr::str_replace_all(segment, "\\<b\\>|\\</b\\>", ""),
      question = stringr::str_replace_all(question, "\\<br\\>", " "),
      dplyr::across(dplyr::matches("sample|weighted"), f1),
      dplyr::across(dplyr::matches("vertical|horizontal"), f2)
    ) |>
    dplyr::rename_with(~ paste(., "%"), dplyr::matches("vertical|horizontal"))
  colnames(segments) <- stringr::str_to_title(gsub(
    ".*\\_",
    "",
    colnames(segments)
  ))
  ...
  colnames(summary) <- stringr::str_to_title(gsub(
    ".*\\_",
    "",
    colnames(summary)
  ))
  ...
  for (i in 1:length(audiences)) {
    ...
    mergeCells(
      wb,
      "Demos",
      cols = colstart[i]:(colstart[(i + 1)] - 1),
      rows = 1
    )
    ...
    mergeCells(
      wb,
      "Psychographics",
      cols = psy_colstart[i]:(psy_colstart[(i + 1)] - 1),
      rows = 1
    )
    conditionalFormatting(
      wb,
    ...
    mergeCells(
      wb,
      "Segments",
      cols = seg_colstart[i]:(seg_colstart[(i + 1)] - 1),
      rows = 1
    )
    conditionalFormatting(
      wb,
  ...
  centerStyle <- createStyle(
    ...
    fontSize = 10,
    fgFill = '#679DD0',
    border = c("left", "right", "top"),
    borderColour = '#4F81BD'
  )
  bodyStyle <- createStyle(halign = "center", fontSize = 10)
  tmpx <- "\U2B9f"
  indexStyle <- createStyle(
    halign = "right",
    border = c("right"),
    borderColour = '#4F81BD'
  )
  oneColStyle <- createStyle(
    halign = "left",
    fontSize = 10,
    textDecoration = "bold"
  )
  ...
  addStyle(
    ...
    indexStyle,
    rows = 1:(nrow(psychographics) + 2),
    cols = psy_indices[1:length(psy_indices) - 1],
    gridExpand = TRUE,
    stack = TRUE
  )
  addStyle(
  ...
  addStyle(
    ...
    indexStyle,
    rows = 3:(nrow(segments) + 2),
    cols = seg_indices[1:length(seg_indices) - 1],
    gridExpand = TRUE,
    stack = TRUE
  )
  addStyle(
